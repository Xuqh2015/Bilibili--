...
- (void)setImages:(NSArray *)images withName:(NSArray *)name
{
    ...
    NSArray *checked = [[NSUserDefaults standardUserDefaults] valueForKey:@"filter"];
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    for (NSString *path in checked) {
        [dict setValue:[NSNumber numberWithBool:YES] forKey:path];
    }
    for (int i = 0 ; i < [name count];i++) {
        ...
        if ([dict valueForKey:[name objectAtIndex:i]]!=nil) {
